...
import {
  setCurrentFeature,
  setCurrentPage,
} from '../redux/slices/featureKeysSlice'
...
const AppSideBar = ({ onToggleSidebar }: AppSideBarProps) => {
  ...
  const { currentFeature, currentPage } = useAppSelector(
    (state) => state.featureKeys,
  )
      ...
      menuConfig.forEach((item) => {
        if (item.children && item.children.length > 0) {
          ...
          const hasActiveChild = item.children.some((child) => {
            if (child.page) {
              return (
                currentFeature === child.feature && currentPage === child.page
              )
            }
  ...
  return (
    <div className="drawer-side">
      ...
      ></label>
      <aside className="w-40 h-full bg-base-200 relative overflow-y-auto border-r border-base-300">
        {/* Logo 区域 */}
