...
import IframePlayer, { type IframePlayerRef } from './IframePlayer'
...
const Player = memo(
  forwardRef<PlayerRef, PlayerProps>(
    ...
    // iframe 播放器引用
    const iframePlayerRef = useRef<IframePlayerRef>(null)

    // 暴露播放器控制方法
    useImperativeHandle(ref, () => ({
      seek: (time: number) => {
        if (isUrlResource && type === 'video') {
          // URL 资源：通过 iframe 播放器控制
          iframePlayerRef.current?.seek(time)
          return
        }
        // 本地文件资源：直接控制播放器
      ...
      onTimeUpdate: (callback: (time: number) => void) => {
        ...
        if (isUrlResource) {
          return iframePlayerRef.current?.onTimeUpdate(callback) ?? (() => {})
        }
